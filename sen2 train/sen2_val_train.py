...
dropout_rate = 0.5
is_training = True
x1 = tf.placeholder(tf.float32, [None, 32, 32, 10], name="x1")
...
# logits, _ = inception_resnet_v2.inception_resnet_v2(x1, class_num)
# net, _ = resnet_v1.resnet_v1_50(x1)
...
# logits, _ = vgg.vgg_16(x1, class_num, True, dropout_rate)
# logits = model_low.model(x1, is_training, dropout_rate, class_num, weight_decay)
...
with tf.Session() as sess:
    ...
    saver.restore(sess, './checkpoint/pre_train_sen2_resnet50_7.ckpt')
    # sess.run(tf.global_variables_initializer())

    for epochs in range(30):
        for i in range(0, split, batch_size):
            start_pos = i
            end_pos = min(i + batch_size, split)
            ...
            sess.run(train_step, feed_dict={x1: val_s2_batch, y_: val_label_batch})
            if (i / batch_size) % 10 == 0:
                summary, l, acc = sess.run([merged, loss, accuracy], feed_dict={x1: val_s2_batch, y_: val_label_batch})
                print("After %d training step(s), loss is %g ,accuracy is %g" % (i / batch_size, l, acc))
            train_writer.add_summary(summary, i)
        saver.save(sess, './checkpoint/val_sen2_resnet_model_' + str(epochs) + '.ckpt')
        ...
        n_val_samples = s2_validation.shape[0]
        total = 0
        for i in range(split, n_val_samples, batch_size):
            start_pos = i
            ...
            temp = sess.run(accuracy, feed_dict={x1: val_s2_batch, y_: val_label_batch})
            total = total + temp * (end_pos - start_pos)

        total = total / (n_val_samples - split)
