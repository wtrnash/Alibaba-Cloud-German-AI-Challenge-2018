...
with tf.Session() as sess:
    saver = tf.train.Saver(max_to_keep=31)
    merged = tf.summary.merge_all()
    train_writer = tf.summary.FileWriter('./train_model_demo', sess.graph)
    saver.restore(sess, './checkpoint/double_pre_vgg_resnet_model_5.ckpt')
    ...
    for epochs in range(31):
        ...
        saver.save(sess, './checkpoint/val_pretrain_resnet_vgg_model_' + str(epochs) + '.ckpt')
        # 计算验证集准确率
        pred_y = []
        train_val_y = np.argmax(label_validation, axis=1)
        batch_size = 128
