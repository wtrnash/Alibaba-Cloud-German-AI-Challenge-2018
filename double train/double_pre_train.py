...
with slim.arg_scope(vgg.vgg_arg_scope(weight_decay)):
    y2, _ = vgg.vgg_16(x2, class_num, True, dropout_rate)
...
with tf.Session() as sess:
    ...
    # 运行该函数
    init_fn(sess)
    init_fn = slim.assign_from_checkpoint_fn('./pre_trained_model/vgg_16.ckpt', variables_to_restore,
                                             ignore_missing_vars=True)
    # 运行该函数
    init_fn(sess)
    ...
    for epochs in range(30):
        for i in range(0, n_sampels, batch_size):
            ...
            train_s1_batch = normalize(train_s1_batch)
            train_s2_batch = normalize(train_s2_batch)

            label_batch = train_label[start_pos:end_pos]

            sess.run(train_step, feed_dict={x1: train_s1_batch, x2: train_s2_batch, y_: label_batch})
            if (i / batch_size) % 10 == 0:
                summary, l, acc = sess.run([merged, loss, accuracy], feed_dict={x1: train_s1_batch, x2: train_s2_batch, y_: label_batch})
                print("After %d training step(s), loss is %g ,accuracy is %g" % (i / batch_size, l, acc))
            train_writer.add_summary(summary, i)

        saver.save(sess, './checkpoint/double_pre_vgg_resnet_model_' + str(epochs) + '.ckpt')
        # 计算验证集准确率
        pred_y = []
